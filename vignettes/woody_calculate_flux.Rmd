...
where $W$=waiting time between two wood occurrences and 3600 seconds=1 hour.

To transform this estimate into an actual **hourly flux** one has to be cautious as to which **sampling resolution** to use. Indeed, in Wdata, the sampling is particularly important when flux is important (since each observation corresponds to a piece of wood). Thus predicting an hourly flux without overestimating it requires to rely on the distribution of W rather than Y and taking into account properties of **mathematical distributions**, in particular the variance of Y ($\sigma$).

For now, we estimate roughly that $\sigma=1.5$.
...
tib_WpQp=tib_WpQc %>%
  mutate(Qdata=purrr::map(.x=Qdata, ~interp_Qdata(.x, timescale="hours")))%>%
...
tib_WhQh=tib_WpQp %>%
  ...
  mutate(Qdata=purrr::map(.x=Qdata,
                          ~summarise_period(data=.x,type="Qdata",period="hour", sigma=1.5))
  )
